...
| 异环 | `/委托绑定 异环` 或 `/委托绑定 异环 UID` | 活跃度、本性像素、都市活力 |
...
异环使用塔吉多账号短信登录。发送 `/委托绑定 异环` 后，机器人会把绑定教程私聊给你。账号里有多个异环角色时，建议直接带 UID：
...
- 异环提醒改为检查活跃度；周日会额外检查都市活力是否清完。
- 查询和提醒文案改为“娜娜米提醒”风格，去掉不清楚的英文完成提示。
